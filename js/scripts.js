...
const solution = {
    1: {
        groupId: 1,
        ids: ["1", "2", "3", "4"],
        texts: ["Cono", "Lengua", "Música", "Tecno"],
        color: "#579c1e",
        title: "Asignaturas del colegio",
        emoji: "🟩",
    },
    2: {
        groupId: 2,
        ids: ["5", "6", "7", "8"],
        texts: ["Sucesión", "Amigos", "El cable", "Sinverguenzas"],
        color: "#1e559c",
        title: "Títulos de series traducidos literalmente",
        emoji: "🟦",
    },
    3: {
        groupId: 3,
        ids: ["9", "10", "11", "12"],
        texts: ["Humano", "Recodo", "Garduña", "Abrazo"],
        color: "#c230b6",
        title: "Contienen partes del cuerpo humano",
        emoji: "🟪",
    },
    4: {
        groupId: 4,
        ids: ["13", "14", "15", "16"],
        texts: ["Martillo", "Espada", "Globo", "Payaso"],
        color: "#D4C712",
        title: "Peces",
        emoji: "🟨",
    },
...
const dateText = `Jueguito del ${String(day).padStart(2, "0")} de ${months[date.getMonth()]
    } de ${date.getFullYear()}`;
...
function restoreState() {
    if (!gameData.final) {
        ...
        document.querySelector("#messages").innerHTML =
            "Selecciona cuatro casillas, tienes <b>" +
            (4 - gameData.errorCount) +
            "</b> intentos.";
    } else {
        gameData.checks = 0;
...
function printTodayResults() {
    ...
    let linkText =
        "whatsapp://send?text=¡Vaya juegazo!%0a" +
        emojis +
        "%0aJuega aquí: " +
        encodedURL;
    let telegramText =
    ...
    const statsHTML = `
        ...
        <ul id="stats">
            <li><span>Sin fallos:</span> <span>${globalStats.buckets[0]} (${pct(
        globalStats.buckets[0]
    )})</span></li>
            <li><span>1 fallo:</span> <span>${globalStats.buckets[1]} (${pct(
        globalStats.buckets[1]
    )})</span></li>
            <li><span>2 fallos:</span> <span>${globalStats.buckets[2]} (${pct(
        globalStats.buckets[2]
    )})</span></li>
            <li><span>3 fallos:</span> <span>${globalStats.buckets[3]} (${pct(
        globalStats.buckets[3]
    )})</span></li>
            <li><span>No resuelto:</span> <span>${globalStats.buckets.fail
        } (${pct(globalStats.buckets.fail)})</span></li>
        </ul>
